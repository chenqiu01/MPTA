...
def build_limitholdem_tree(num_players, num_of_suits, num_of_ranks, betting_parameters):
    """
    Build a tree for the game of limitholdem with a given number of players, suits, ranks and betting parameters.
    """
    ...
    all_card = init_standard_deck()
    
    # hands = build_all_possible_hands(num_players, all_card)
    # print("rank:", (hands[0][0].rank))
    # print("suit:", (hands[0][0].suit))
    # print("number of hands:", len(hands))

    hands = [all_card[0], all_card[1], all_card[2], all_card[3], all_card[4], all_card[5], all_card[6], all_card[7], all_card[8], all_card[9], all_card[10]]
    for i in range(11):
        print("card" + str(i) + " is: " + hands[i].suit + hands[i].rank)
    ...
    for hand in hands:
        ...
        all_nodes.append(n)
        all_nodes += build_limitholdem_hand_tree(hand, 0, 0, n, betting_parameters, tree)
...
def build_limitholdem_hand_tree(hand, current_player, current_round, current_node, betting_parameters, tree):
    """
    Recursively build the subtree for the limitholdem game where the hand is fixed.
    """
...
def build_all_possible_hands(num_players, cards):
    """
    Build all the possible hands for the game of limitholdem with a given number of players and a given set of cards.
...
def limitholdem_utility(hand, previous_moves, betting_parameters):
    """
    Get the utility of a limitholdem game given the hand, how the players have played and the betting parameters.
...
#多人转换算法（0704+）
#rank仅用于团队成员转化
#limitholdem 动作为3个，raise(r),fold(f),call(c)
def pub_team_convert_limitholdem(ori_tree,rank,num_players):
    ...
    version = 4
    if version == 2:
        pub_convert_limitholdem_v2(ori_tree.root,None,new_tree,rank)
        new_tree.convert_version =2
    elif version == 3:
        pub_convert_limitholdem_v3(ori_tree.root,None,new_tree,rank, [0]*ori_tree.numOfPlayers)
        new_tree.convert_version =3
    elif version == 4:
        pub_convert_limitholdem_v4(ori_tree.root,None,new_tree,rank)
    ...
    root=new_tree.root
    if version == 4:
        new_tree.print_tree("conver_limitholdem_tree_v"+str(version))
        return new_tree
...
#rank仅用于团队成员转化 
def pub_convert_limitholdem_v4(ori_node,parent_node,new_tree,rank):
  #叶节点
  if ori_node.isLeaf():
  ...
  #机会节点和敌手节点
    ...
    if ori_node.isChance():#limitholdem也只有根节点会是chance
        for i in range(len(ori_node.children)):
            pub_convert_limitholdem_v4(ori_node.children[i], new_tree.root, new_tree, rank)
    if ori_node.player == 0:
        ...
        for i in range(len(ori_node.children)):
            pub_convert_limitholdem_v4(ori_node.children[i], new_node, new_tree,rank)
...
def pub_convert_limitholdem_v3(ori_node,parent_node,new_tree,rank,flags):
  #叶节点
  if ori_node.isLeaf():
    new_tree.addLeaf(parent_node, ori_node.utility,actionName =ori_node.incoming_action_name)
  ...
  #机会节点和敌手节点,针对Kuhn
    ...
    if ori_node.isChance():#limitholdem也只有根节点会是chance
        for i in range(len(ori_node.children)):
            pub_convert_limitholdem_v3(ori_node.children[i], new_tree.root, new_tree, rank, [0]*new_tree.ori_numOfPlayers)
    if ori_node.player == 0:
        new_node = new_tree.addNode(ori_node.player,parent = parent_node,actionName=ori_node.incoming_action_name)
        new_node.ori_player = ori_node.player
        new_node.known_information=ori_node.known_information
        for i in range(len(ori_node.children)):
            pub_convert_limitholdem_v3(ori_node.children[i], new_node, new_tree,rank, flags)
  ...
  #团队玩家
    ...
    if flags[ori_node.player] == 0:
        ...
        for i in range(rank):
            ...
            #寻找原树的每一个子节点，通过动作来将原树该动作下的子节点进行转换，并接到转换后的树上
            for i in range(len(ori_node.children)) :
                pub_convert_limitholdem_v3(ori_node.children[i],temp_chance_node,new_tree,rank,flags)
...
#生成处方,返回['AAA','AAB',...,'BBB']类型的处方,limitholdem需要修改,不同类游戏的num2act构建自己的数字对字符的动作对应，使用时选择自己的legal_action
...
hand=[]
index=0
def pub_convert_limitholdem_v2(ori_node,parent_node,new_tree,rank):
    ...
    #机会节点和敌手节点,针对Kuhn
        ...
        if ori_node.isChance():#Kuhn只有根节点会是chance
            for i in range(len(ori_node.children)):
                pub_convert_limitholdem_v2(ori_node.children[i], new_tree.root, new_tree,rank)
        if ori_node.player == 0:
            ...
            new_node.ori_player = ori_node.player
            for i in range(len(ori_node.children)):
                pub_convert_limitholdem_v2(ori_node.children[i], new_node, new_tree,rank)
        ...
        for i in range(len(prescription)):
            ...
            for j in range(len(legal_actions)):
                if prescription[i][hand[ori_node.player]] == legal_actions[j]:
                    pub_convert_limitholdem_v2(ori_node.children[j],temp_chance_node,new_tree,rank)
            #至此，转换中间的chance节点（temp_chance_node）转换完毕
